...
randN = randi(2);%aleatorio entre 1 y 2
if(randN == 1)
    %mom's genes first
    kid.Coord = cat(2,mom.Coord(:,1:crossPoint),dad.Coord(:,crossPoint+1:end));
elseif(randN ==2)
    %dad's genes first
    kid.Coord = cat(2,dad.Coord(:,1:crossPoint),mom.Coord(:,crossPoint+1:end));
end
